...
import { StatusBadge, EmptyState, ErrorBanner, isSafeUrl, hasBengaliText } from '../components/Shared'
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Filter Pills */}
        <div className="flex gap-1.5 p-1 bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#2A3348] rounded-xl self-start sm:self-auto shadow-xs">
      ...
      {loading ? (
        <div className="text-sm text-slate-500 dark:text-slate-400 py-16 text-center">Loading…</div>
      ) : filtered.length === 0 ? (
        <EmptyState
          icon={Inbox}
        ...
        <div className="space-y-4">
          {filtered.map((s) => {
            const isTitleBengali = hasBengaliText(s.tasks?.title)
            const isProofBengali = hasBengaliText(s.proof_text)
            const isRejectionBengali = hasBengaliText(s.rejection_reason)
            return (
              <div
                key={s.id}
                className="card p-5 sm:p-6 rounded-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#2A3348] shadow-xs hover:border-slate-300 dark:hover:border-slate-700 transition-all"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p
                      lang={isTitleBengali ? 'bn' : 'en'}
                      className={`font-bold text-base text-slate-900 dark:text-[#F1F5F9] break-words ${
                        isTitleBengali ? 'font-bengali leading-[1.6]' : 'font-sans'
                      }`}
                      style={isTitleBengali ? { fontFamily: "'Hind Siliguri', 'Inter', sans-serif" } : undefined}
                    >
                      {s.tasks?.title}
                    </p>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                      Worker: <span className="font-semibold text-slate-700 dark:text-slate-200">{s.worker_name || 'Worker'}</span> ·{' '}
                ...
                <div className="mt-3.5 pt-3 border-t border-slate-200/80 dark:border-[#2A3348]/60">
                  <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">
                    Worker's Proof Submission:
                  </p>
                  <p
                    lang={isProofBengali ? 'bn' : 'en'}
                    className={`text-xs sm:text-sm font-normal text-slate-800 dark:text-slate-200 break-words ${
                      isProofBengali ? 'font-bengali leading-[1.65]' : 'font-sans leading-relaxed'
                    }`}
                    style={isProofBengali ? { fontFamily: "'Hind Siliguri', 'Inter', sans-serif" } : undefined}
                  >
                    {s.proof_text}
                  </p>
                </div>
                ...
                {s.proof_url && isSafeUrl(s.proof_url) && /\.(jpe?g|png|webp|gif)$/i.test(s.proof_url) ? (
                  <a href={s.proof_url} target="_blank" rel="noreferrer" className="block mt-3.5">
                    <img
                      ...
                      className="rounded-xl max-h-64 object-contain border border-slate-200 dark:border-[#2A3348] bg-slate-50 dark:bg-slate-900"
                    />
                  </a>
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400 mt-2.5 inline-block break-all">
                    {s.proof_url}
                  </span>
                ) : null}

                {s.status === 'pending' && (
                  <div className="mt-4 pt-4 border-t border-slate-200/80 dark:border-[#2A3348]/60">
                    {rejectingId === s.id ? (
                      <div className="space-y-3">
                        <textarea
                          className="w-full rounded-xl border border-slate-200 dark:border-[#2A3348] bg-white dark:bg-[#0B0F17] px-3.5 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-[#F1F5F9] outline-none transition focus:border-brand-primary focus:ring-1 focus:ring-brand-primary font-sans"
                          rows={2}
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="px-4 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-[#2A3348] text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                          >
                            Cancel
                          </button>
                        </div>
                ...
                {s.status === 'rejected' && s.rejection_reason && (
                  <div className="mt-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 px-3.5 py-2.5 text-xs sm:text-sm text-rose-700 dark:text-rose-300 flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <span
                        lang={isRejectionBengali ? 'bn' : 'en'}
                        className={isRejectionBengali ? 'font-bengali leading-[1.6]' : 'font-sans'}
                        style={isRejectionBengali ? { fontFamily: "'Hind Siliguri', 'Inter', sans-serif" } : undefined}
                      >
                        {s.rejection_reason}
                      </span>
                    </div>
