...
export default function StatCard({ icon: Icon, label, value, tone = 'green', hint }) {
  return (
    <div className="card p-5 sm:p-6 flex items-start justify-between rounded-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#2A3348] shadow-xs hover:border-emerald-500/50 dark:hover:border-[#2A3348] hover:-translate-y-0.5 hover:shadow-md transition-all duration-200">
      <div>
        <p className="text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">
          {label}
        </p>
        <p className="mt-1.5 text-2xl sm:text-3xl font-display font-extrabold text-slate-900 dark:text-[#F1F5F9] tracking-tight">
          {value}
        </p>
        {hint && (
